...
class hybridCnnTrans(nn.Module):
    def __init__(self, input_c=3, c=16, num_classes=1, meta_node_num=4, layers=7, dp=0,
        ...
        print(f"Initializing model with {self.layers} layers and {self.num_transformer_connections} transformer connections")
        ...
        # Dynamic transformer blocks based on number of connections
        self.transformer_blocks = nn.ModuleList()
        self.position_embeds = nn.ModuleList()
        
        # Create transformer blocks dynamically based on num_transformer_connections
        transformer_configs = [
        ...
        # Create individual attributes for backward compatibility with forward method
        for i in range(len(self.position_embeds)):
            setattr(self, f'position_embed_{i}', self.position_embeds[i])
            setattr(self, f'encoder_Detrans_{i}', self.transformer_blocks[i])
    ...
    def load_alphas(self, alphas_dict):
        ...
        # Load transformer connections if available (for backward compatibility)

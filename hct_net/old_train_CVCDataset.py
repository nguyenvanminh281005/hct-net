...
def configure_ablation_study(args):
    """
    Cấu hình ablation study cho 8 trường hợp (dice luôn có).
    
    8 trường hợp:
    1. all: dice + transformer + complexity + entropy
    ...
    Returns:
        dict with flags for each loss component
    """
    config = {
        'use_dice': True,  # LUÔN CÓ
        'use_transformer': False,
        'use_complexity': False,
        'use_entropy': False,
    }
    
    # Ánh xạ ablation mode
    if args.ablation_mode == 'all':
        config['use_transformer'] = True
        config['use_complexity'] = True
        config['use_entropy'] = True
        
    elif args.ablation_mode == 'no_transformer':
        config['use_transformer'] = False
        config['use_complexity'] = True
        config['use_entropy'] = True
        
    elif args.ablation_mode == 'no_complexity':
        config['use_transformer'] = True
        config['use_complexity'] = False
        config['use_entropy'] = True
        
    elif args.ablation_mode == 'no_entropy':
        config['use_transformer'] = True
        config['use_complexity'] = True
        config['use_entropy'] = False
        
    elif args.ablation_mode == 'only_transformer':
        config['use_transformer'] = True
        config['use_complexity'] = False
        config['use_entropy'] = False
        
    elif args.ablation_mode == 'only_complexity':
        config['use_transformer'] = False
        config['use_complexity'] = True
        config['use_entropy'] = False
        
    elif args.ablation_mode == 'only_entropy':
        config['use_transformer'] = False
        config['use_complexity'] = False
        config['use_entropy'] = True
    ...
    # Override bằng custom flags nếu có
    if args.use_transformer_loss is not None:
        config['use_transformer'] = args.use_transformer_loss
    if args.use_complexity_loss_component is not None:
        config['use_complexity'] = args.use_complexity_loss_component
    if args.use_entropy_loss is not None:
        config['use_entropy'] = args.use_entropy_loss
...
def hierarchical_pareto_optimization(dice_loss, entropy_loss, complexity_loss, transformer_loss, 
                                     ...
                                     architecture, ablation_config):
    """
    Hierarchical Pareto Optimization with 2 levels + ABLATION STUDY SUPPORT.
    
    **ABLATION**: Chỉ cập nhật Pareto front cho các objectives được bật.
    
    Level 1 (Within-Group Pareto):
        - Group 1 Pareto: dice_loss vs entropy_loss (nếu cả 2 đều bật)
        - Group 2 Pareto: complexity_loss vs transformer_loss (nếu cả 2 đều bật)
    ...
    Args:
        ...
        ablation_config: dict with flags for each loss component
        
    Returns:
        tuple: (updated_group1_front, updated_group2_front, updated_top_level_front)
    """
    # === LEVEL 1: Within-Group Pareto (CHỈ NẾU CẢ 2 OBJECTIVES ĐƯỢC BẬT) ===
    
    # Group 1: Accuracy (dice vs entropy)
    if ablation_config['use_dice'] and ablation_config['use_entropy']:
    ...
    if ablation_config['use_complexity'] and ablation_config['use_transformer']:
    ...
    # === LEVEL 2: Top-Level Pareto (CHỈ NẾU CÓ ÍT NHẤT 2 NHÓM ACTIVE) ===
    
    # Tính aggregated values dựa trên ablation config
    accuracy_components = []
    if ablation_config['use_dice']:
        accuracy_components.append(dice_loss)
    if ablation_config['use_entropy']:
        accuracy_components.append(entropy_loss)
    
    complexity_components = []
    if ablation_config['use_complexity']:
        complexity_components.append(complexity_loss)
    if ablation_config['use_transformer']:
        complexity_components.append(transformer_loss)
    
    # Chỉ làm top-level Pareto nếu có đủ thành phần
    if len(accuracy_components) > 0 and len(complexity_components) > 0:
        aggregated_accuracy = sum(accuracy_components) / len(accuracy_components)
        aggregated_complexity = sum(complexity_components) / len(complexity_components)
...
def compute_pareto_guided_loss(objectives, dice_loss, complexity_loss, 
    ...
    **ABLATION STUDY SUPPORT**: Chỉ tính các loss được bật theo ablation_config.
    ...
    """
    ablation = args.ablation_config
    
    if len(objectives) == 2:
        # === 2D TOP-LEVEL PARETO với ABLATION SUPPORT ===
        target_aggregated_accuracy, target_aggregated_complexity = objectives
        
        # === TÍNH CURRENT AGGREGATED DỰA TRÊN ABLATION CONFIG ===
        current_aggregated_accuracy = 0.0
        accuracy_components = 0
        
        # Dice luôn có
        if ablation['use_dice']:
            current_aggregated_accuracy += dice_loss.detach().item()
            accuracy_components += 1
        
        # Entropy (nếu bật)
        if ablation['use_entropy']:
            current_aggregated_accuracy += entropy_loss.detach().item()
            accuracy_components += 1
        
        # Normalize accuracy
        if accuracy_components > 0:
            current_aggregated_accuracy /= accuracy_components
        
        # === COMPLEXITY GROUP ===
        current_aggregated_complexity = 0.0
        complexity_components = 0
        
        if ablation['use_complexity']:
            current_aggregated_complexity += complexity_loss.detach().item()
            complexity_components += 1
        
        if ablation['use_transformer']:
            current_aggregated_complexity += transformer_loss.detach().item()
            complexity_components += 1
        
        # Normalize complexity
        if complexity_components > 0:
            current_aggregated_complexity /= complexity_components
        
        # === COMPUTE LOSS DỰA TRÊN CÁC THÀNH PHẦN ĐƯỢC BẬT ===
        total_loss = 0.0
        active_components = 0
        ...
        if ablation['use_entropy']:
            total_loss += 0.3 * entropy_loss  # Entropy ít quan trọng hơn
            active_components += 1
        
        if ablation['use_complexity']:
            total_loss += 0.2 * complexity_loss
            active_components += 1
        
        if ablation['use_transformer']:
            total_loss += transformer_loss  # Transformer loss đã âm (reward)
            active_components += 1
        
        # Normalize
        if active_components > 0:
            loss = total_loss / active_components
        else:
            # Fallback: chỉ dice
            loss = dice_loss
...
def main(args):
    ...
    ablation_config = configure_ablation_study(args)
    args.ablation_config = ablation_config
    
    #################### init logger ###################################
    log_dir = './search_exp/' + '/{}'.format(args.model) + \
              '/{}'.format(args.dataset) + '/ablation_{}_{}'.format(args.ablation_mode, time.strftime('%Y%m%d-%H%M%S'))
    ...
    # Initialize Weights & Biases with ablation info
    wandb.init(
        project="hct-net-ablation",
        name=f"{args.model}_{args.dataset}_ablation_{args.ablation_mode}_{time.strftime('%Y%m%d-%H%M%S')}",
        config=vars(args),
        dir=log_dir,
        tags=[args.ablation_mode, f"dice_{'on' if ablation_config['use_dice'] else 'off'}", 
              f"trans_{'on' if ablation_config['use_transformer'] else 'off'}",
              f"comp_{'on' if ablation_config['use_complexity'] else 'off'}",
              f"ent_{'on' if ablation_config['use_entropy'] else 'off'}"]
    )
    ##################### init device #################################
    ...
    logger.info(f"Dice Loss:        {'✓ ENABLED' if ablation_config['use_dice'] else '✗ DISABLED'} (always enabled)")
    logger.info(f"Transformer Loss: {'✓ ENABLED' if ablation_config['use_transformer'] else '✗ DISABLED'}")
    logger.info(f"Complexity Loss:  {'✓ ENABLED' if ablation_config['use_complexity'] else '✗ DISABLED'}")
    logger.info(f"Entropy Loss:     {'✓ ENABLED' if ablation_config['use_entropy'] else '✗ DISABLED'}")
    logger.info("="*80)
    logger.info("HIERARCHICAL PARETO OPTIMIZATION INITIALIZED")
    logger.info("="*80)
    logger.info("Level 1 - Within-Group Pareto (only if both objectives enabled):")
    if ablation_config['use_dice'] and ablation_config['use_entropy']:
        logger.info("  Group 1 (Accuracy): dice_loss vs entropy_loss")
    else:
        logger.info("  Group 1 (Accuracy): SKIPPED (not enough objectives)")
    
    if ablation_config['use_complexity'] and ablation_config['use_transformer']:
        logger.info("  Group 2 (Complexity): complexity_loss vs transformer_loss")
    else:
        logger.info("  Group 2 (Complexity): SKIPPED (not enough objectives)")
    
    logger.info("Level 2 - Top-Level Pareto:")
    logger.info("  Uses only ENABLED components for aggregation")
    logger.info("="*80 + "\n")
    ...
    for epoch in range(start_epoch, sp_epoch):
        ...
        if args.enable_complexity_loss:
            wandb.log({
                ...
                'train/arch_complexity_loss': arch_complexity_loss_avg,
        ...
        wandb.log({
            'pareto/top_level_count': len(top_level_pareto_front),
            'pareto/group1_count': len(group1_pareto_front),
            'pareto/group2_count': len(group2_pareto_front),
        })
        ...
        if len(top_level_pareto_front) > 0:
            ...
            wandb.log({
                'pareto/best_top_agg_acc': best_top_sol[0][0],
                'pareto/best_top_agg_comp': best_top_sol[0][1],
            })
        ...
        if len(group1_pareto_front) > 0:
            ...
            wandb.log({
                'pareto/best_group1_dice': best_group1_sol[0][0],
                'pareto/best_group1_entropy': best_group1_sol[0][1],
            })
        ...
        if len(group2_pareto_front) > 0:
            ...
            wandb.log({
                'pareto/best_group2_complexity': best_group2_sol[0][0],
                'pareto/best_group2_transformer': best_group2_sol[0][1],
            })
...
def train(args, train_queue, val_queue, model, criterion, optimizer_weight, optimizer_arch, 
    ...
    for step, (input, target, _) in enumerate(train_queue):
        ...
        if train_arch:
            ...
            # Update HIERARCHICAL Pareto fronts (Level 1 + Level 2) với ABLATION SUPPORT
            ...
            if target_objectives is not None and args.enable_complexity_loss:
                ...
                if args.enable_complexity_loss:
                    ...
                    group1_loss = 0.8 * accuracy_loss + 0.2 * arch_complexity_loss
...
if __name__ == '__main__':
    ...
    # === ABLATION STUDY: 8 trường hợp (2^3) cho transformer, complexity, entropy (dice luôn có) ===
    parser.add_argument('--ablation_mode', type=str, default='all', 
                        choices=['all', 'no_transformer', 'no_complexity', 'no_entropy',
                                'only_transformer', 'only_complexity', 'only_entropy', 'none'],
                        help='Ablation study mode:\n'
                             'all: dice + transformer + complexity + entropy\n'
                             'no_transformer: dice + complexity + entropy\n'
                             'no_complexity: dice + transformer + entropy\n'
                             'no_entropy: dice + transformer + complexity\n'
                             'only_transformer: dice + transformer\n'
                             'only_complexity: dice + complexity\n'
                             'only_entropy: dice + entropy\n'
                             'none: chỉ dice (baseline)')
    
    # Custom ablation: bật/tắt riêng lẻ (cho trường hợp đặc biệt)
    parser.add_argument('--use_transformer_loss', type=bool, default=None, help='override transformer loss (None = follow ablation_mode)')
    parser.add_argument('--use_complexity_loss_component', type=bool, default=None, help='override complexity loss (None = follow ablation_mode)')
    parser.add_argument('--use_entropy_loss', type=bool, default=None, help='override entropy loss (None = follow ablation_mode)')

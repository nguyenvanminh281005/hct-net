...
def mean_iou(y_true_in, y_pred_in, print_table=False):
    ...
    if True: #not np.sum(y_true_in.flatten()) == 0:
        ...
        def precision_at(threshold, iou):
            ...
            true_positives = np.sum(matches, axis=1) == 1   #Missed objec Correct objects
            false_positives = np.sum(matches, axis=0) == 0  # ts
...
def iou_score_upp(output, target):
    smooth = 1e-5
...
def dice_coef_upp(output, target):
    smooth = 1e-5
